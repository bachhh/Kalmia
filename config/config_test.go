...
func TestCookieConfig_UnmarshalJSON(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			jsonData: []byte(`{"domain": "example.com", "path": "/", "age": "24h", "secure": true, "sameSite": "Lax", "httpOnly": true}`),
			expected: config.CookieConfig{
				Domain:   "example.com",
				Path:     "/",
				Age:      "24h",
		...
		},
		{
			...
			jsonData: []byte(`{"domain": "example.com", "path": "/", "age": "24h", "secure": true, "sameSite": "Strict", "httpOnly": true}`),
			expected: config.CookieConfig{
				Domain:   "example.com",
				Path:     "/",
				Age:      "24h",
		...
		},
		{
			...
			jsonData: []byte(`{"domain": "example.com", "path": "/", "age": "24h", "secure": true, "sameSite": "None", "httpOnly": true}`),
			expected: config.CookieConfig{
				Domain:   "example.com",
				Path:     "/",
				Age:      "24h",
