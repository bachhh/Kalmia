...
interface EditPageGroupModalProps {
  name: string;
  label: string;
  updateData: (name: string, label: string, id: number) => void;
  id: number;
}

export default function EditPageGroupModal({
  name,
  label,
  updateData,
  id,
}: EditPageGroupModalProps) {
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key === "Enter") {
      ...
      updateData(editName, editLabel, id);
    }
  };
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        key="edit-page-group-container"
      >
        <div
          className="relative p-4 w-full max-w-xl"
          key="edit-page-group-container-0"
        >
          <div className="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div className="grid gap-4 mb-4">
              ...
              <button
                onClick={() => updateData(editName, editLabel, id)}
                type="button"
